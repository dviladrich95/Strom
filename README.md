...
The `freq` (time resolution) is set per-script and is not part of the shared house configuration — see the individual case-study scripts under `case_study/` for the value used in each run. Otherwise the main script will generate an empty json file that will fall back to the default values defined in `strom/optimization_utils.py`.
